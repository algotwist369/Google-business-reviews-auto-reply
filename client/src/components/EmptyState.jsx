...
const EmptyState = memo(function EmptyState({ onClearFilters }) {
  const copy = useMemo(
    () => ({
      ...
      body: 'No reviews match your current filter settings.',
      cta: 'Clear Filters'
    }),
    []
  );
  ...
  return (
    <div className="text-center py-12 sm:py-16 bg-white rounded-xl border border-gray-100 shadow-sm animate-in fade-in zoom-in-95 duration-300">
      ...
      <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">{copy.title}</h3>
      <p className="text-sm sm:text-base text-gray-500 px-4 mb-4">{copy.body}</p>
      <button
