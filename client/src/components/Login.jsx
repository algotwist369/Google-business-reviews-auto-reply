import React, { memo, useMemo } from 'react';
import { CheckCircle2 } from 'lucide-react';
import { API_URL } from '../utils/constants';

const highlightItems = Object.freeze([
  'Auto-detects new & unreplied Google reviews',
  'LLM powered, human-quality replies in your tone',
  'Configurable delays, tones, and sentiment rules',
  'Keeps running until you pause it - no manual babysitting'
]);

const Login = memo(function Login() {
  const highlights = useMemo(() => highlightItems, []);
  const authUrl = useMemo(() => `${API_URL}/auth/google`, []);

  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100 text-gray-900 flex items-center justify-center px-4 py-10">
      <div className="max-w-5xl w-full grid gap-6 lg:gap-8 lg:grid-cols-2 items-center">
        {/* Overview */}
        <section className="bg-white border border-gray-100   p-8  ">
          <img src="/logo.png" alt="Rama logo" className="w-20 h-20 object-contain mb-6" loading="eager" />
          <p className="text-xs uppercase tracking-[0.4em] text-gray-400">Auto Reply OS</p>
          <h1 className="text-3xl font-bold text-gray-900 mt-2">Rama Review Manger</h1>
          <p className="text-gray-600 mt-4 leading-relaxed text-base">
            The always-on assistant for Google reviews. Rama detects new feedback, understands sentiment, and responds with
            short, natural replies that match your voice-on autopilot.
          </p>

          <div className="mt-6 space-y-3">
            {highlights.map((item) => (
              <div key={item} className="flex items-center gap-3 text-gray-700 text-sm sm:text-base">
                <CheckCircle2 className="text-emerald-500 flex-shrink-0" size={18} aria-hidden="true" />
                <span>{item}</span>
              </div>
            ))}
          </div>
        </section>

        {/* Sign-in card */}
        <section className="bg-white text-gray-900   p-6 sm:p-10   border border-gray-100 flex flex-col justify-center">
          <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <img
              src="/rama-fav.png"
              alt="Rama favicon"
              className="w-8 h-8 sm:w-10 sm:h-10 object-contain"
              loading="lazy"
            />
          </div>
          <h2 className="text-2xl font-bold text-center text-gray-900">Sign in to Rama</h2>
          <p className="text-center text-gray-500 mb-8 text-sm">
            Secure Google OAuth 2.0 authentication. No passwords, no hassle.
          </p>

          <a
            href={authUrl}
            className="block w-full bg-gray-900 hover:bg-black text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:  transform hover:-translate-y-0.5 active:scale-[0.99]"
            aria-label="Sign in with Google"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" aria-hidden="true">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google
          </a>

          <div className="mt-6 text-center text-xs text-gray-500">
            By continuing you agree to the Rama Terms of Service and Privacy Policy.
          </div>
        </section>
      </div>
    </div>
  );
});

Login.displayName = 'Login';

export default Login;

